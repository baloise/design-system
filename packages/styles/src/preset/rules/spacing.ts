 ...
 * Source: 📱 Device › ↔️ Space
 *
 * Each size group (None, Auto, 2XS, …) contributes a set of margin/padding/gap
 * classes referencing the auto-switching `--ds-device-space-*` CSS variable so
 * that the classes respond to breakpoints automatically via the token CSS.
 * `Auto` has no responsive variant, so it falls back to the plain
 * `--ds-device-space-auto` CSS variable. Backward-compat aliases (xx-small,
 * x-small, …) are also emitted.
 */
...
function sv(size: string): string {
  if (size === 'auto') {
    return `var(--ds-device-space-auto)`
  }
...
export function buildSpacingRules(tokensJsonPath: string): {
  ...
  const spaceCategory = (json['📱 Device']?.['↔️ Space'] ?? {}) as Record<string, unknown>
