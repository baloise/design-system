...
function resolveBound(component: object, bound: number | string): number | null {
  const resolvedValue = typeof bound === 'string' ? component[bound as keyof typeof component] : bound
 ...
 */
export function LessThan<T extends object>(maximum: number | Extract<keyof T, string>, options: LessThanOptions = {}) {
  return createBoundValidatorDecorator<T>(
    maximum,
