...
import { PhoneChangeDetail, PhoneCountryChangeDetail, PhoneInputDetail } from './phone-input.interfaces'
import { i18nDsPhoneInput } from './phone-input.i18n'
...
/**
 * Phone input renders an international phone number field with a country picker
...
@Component({
  tag: 'ds-phone-input',
  styleUrl: 'phone-input.host.scss',
  shadow: true,
  formAssociated: true,
})
export class PhoneInput implements DsComponentInterface, FieldInterface {
  ...
  inputId = `ds-phone-input-${PhoneInputIds++}`
  ...
  private warnCountryMismatch(code: string, available: CountryOption[]) {
    ...
    console.warn(`[ds-phone-input] \`country\` "${code}" is not in \`countries\`. Falling back to "${fallback ?? ''}".`)
  }
  ...
  private triggerAccessibleName(selected: CountryOption | undefined, countryName: string): string {
    const i18n = i18nDsPhoneInput[this.language]
    if (!selected) {
