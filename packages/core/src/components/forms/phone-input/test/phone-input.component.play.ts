import { DsPhoneInput, expect, test } from '@helvetia-design/playwright'
...
test.describe('value assigned after load', () => {
  test('should render a value that is assigned after the component has loaded', async ({ page }) => {
    await page.mount(`<ds-phone-input label="Phone number" initial-country="CH"></ds-phone-input>`)
    const phone = new DsPhoneInput(page.locator('ds-phone-input'))

    await phone.assertNationalNumber('')

    await phone.el.evaluate((el: HTMLDsPhoneInputElement) => {
...
test.describe('component', () => {
  ...
  test('should derive a country-specific placeholder and allow an override', async ({ page }) => {
    await page.mount(`
      <ds-phone-input data-testid="derived" label="Phone number" initial-country="CH"></ds-phone-input>
      <ds-phone-input data-testid="custom" label="Phone number" initial-country="CH" placeholder="Custom"></ds-phone-input>
    `)
    const derived = new DsPhoneInput(page.getByTestId('derived'))
    const custom = new DsPhoneInput(page.getByTestId('custom'))
...
test.describe('country picker', () => {
  test('should fire dsCountryChange when a country is selected', async ({ page }) => {
    await page.mount(`<ds-phone-input label="Phone number" countries="CH,DE,FR" initial-country="CH"></ds-phone-input>`)
    const phone = new DsPhoneInput(page.locator('ds-phone-input'))
  ...
  test('should keep typed digits when switching country', async ({ page }) => {
    await page.mount(`<ds-phone-input label="Phone number" countries="CH,DE" initial-country="CH"></ds-phone-input>`)
    const phone = new DsPhoneInput(page.locator('ds-phone-input'))
  ...
  test('should restrict picker options to the countries allow-list', async ({ page }) => {
    await page.mount(`<ds-phone-input label="Phone number" countries="CH,DE"></ds-phone-input>`)
    const phone = new DsPhoneInput(page.locator('ds-phone-input'))
  ...
  test('should warn and fall back when country is outside the allow-list', async ({ page }) => {
    ...
    await page.mount(`<ds-phone-input label="Phone number" countries="CH,DE" country="US"></ds-phone-input>`)
    const phone = new DsPhoneInput(page.locator('ds-phone-input'))

    await expect.poll(() => phone.el.evaluate(el => (el as HTMLDsPhoneInputElement).country)).toBe('CH')
    expect(warnings).toContain('[ds-phone-input] `country` "US" is not in `countries`. Falling back to "CH".')
  })
})
...
test.describe('international calling code input', () => {
  ...
  test('should switch country and remove a typed + calling code from the number field', async ({ page }) => {
    await page.mount(`<ds-phone-input label="Phone number" countries="CH,DE" initial-country="DE"></ds-phone-input>`)
    const phone = new DsPhoneInput(page.locator('ds-phone-input'))
    const countrySpy = await phone.el.spyOnEvent('dsCountryChange')
...
test.describe('disabled', () => {
  test('trigger and number field should be disabled', async ({ page }) => {
    await page.mount(`<ds-phone-input label="Phone number" initial-country="CH" disabled></ds-phone-input>`)
    const phone = new DsPhoneInput(page.locator('ds-phone-input'))
    const inputSpy = await phone.el.spyOnEvent('dsInput')
...
test.describe('readonly', () => {
  test('picker should be disabled while the number field stays usable', async ({ page }) => {
    await page.mount(
      `<ds-phone-input label="Phone number" initial-country="CH" value="+41791234567" readonly></ds-phone-input>`,
    )
    const phone = new DsPhoneInput(page.locator('ds-phone-input'))
...
test.describe('form reset', () => {
  test('should reset to the initial E.164 value', async ({ page }) => {
    await page.mount(`
      <form>
        <ds-phone-input name="phone" label="Phone number" initial-country="CH" value="+41791234567"></ds-phone-input>
