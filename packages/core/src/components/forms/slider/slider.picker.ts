...
import { decimalsFromStep } from './slider.utils'

export interface SliderPickerConfig {
...
export class SliderPickerController {
  private slider: API | undefined
  private config: SliderPickerConfig
  ...
  constructor(config: SliderPickerConfig) {
    this.config = config
    this.init()
  }
  ...
  private init() {
    ...
    this.slider = noUiSlider.create(targetEl, {
      ...
      // the remainder shows the plain track background — see slider.host.scss.
      connect: 'lower',
      tooltips: true,
