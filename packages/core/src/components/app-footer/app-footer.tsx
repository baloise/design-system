...
import {
  APP_FOOTER_CONTAINERS,
  AppFooterContainer,
  AppFooterLanguageChangeDetail,
  AppFooterLink,
  AppFooterSocialLink,
} from './app-footer.interfaces'
import { i18nDsAppFooter } from './app-footer.i18n'
...
@Component({
  tag: 'ds-app-footer',
  styleUrl: 'app-footer.host.scss',
  shadow: true,
})
export class AppFooter implements DsComponentInterface, DsConfigObserver {
  log!: LogInstance
  ...
  @State() legalLinksConfig: { [key in DsLanguage]?: AppFooterLink[] } | undefined
  @State() legalTextConfig: string | undefined
  @State() socialLinksConfig: AppFooterSocialLink[] | undefined
  ...
  private getLanguageLabel(languageCode: DsLanguage): string {
    const labels = i18nDsAppFooter[this.language].languageLabels
  ...
  private renderLinkList(links: AppFooterLink[], className: string) {
    return (
      <ul class={className}>
        {links.map(link => (
  ...
  private renderSocialList(links: AppFooterSocialLink[]) {
    return (
        ...
        >
          <div class="top-row">
            ...
            {!this.hideLanguageSelection && (
              <label class="language-picker" aria-label={i18nDsAppFooter[this.language].languageSelector}>
                <ds-icon name="web" aria-hidden="true" />
                <select data-testid="app-footer-language-select" onChange={this.handleLanguageChange}>
