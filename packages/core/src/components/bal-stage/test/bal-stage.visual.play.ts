...
test('basic', async ({ page }) => {
  ...
  await expectScreenshot(page, image('basic'), { maxDiffPixelRatio: 0.02, fullPage: true })
})

test('large', async ({ page }) => {
  await page.setupVisualTest(`/components/${TAG}/test/${TAG}-large.visual.html`)

  await expectScreenshot(page, image('large'), { maxDiffPixelRatio: 0.02, fullPage: true })
})
