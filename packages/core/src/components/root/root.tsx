...
export class Root implements DsComponentInterface, DsConfigObserver {
  ...
  notifyResize = async () => {
    if (dsBrowser.hasDocument && dsBrowser.hasWindow) {
      ...
      doc.style.setProperty('--ds-root-height', height)
      doc.style.setProperty('--ds-app-height', height)
    }
  }
    ...
    return (
      <Host
        class={{
          'ds-root': true,
          'ds-app': true,
          'ds-root--safari': dsBrowser.isSafari,
          'ds-app--safari': dsBrowser.isSafari,
          'ds-root--touch': dsDevice.hasTouchScreen,
