...
import { generateComponentTags } from './config/generate-component-tags.mjs'
...
export const config: Config = {
  ...
  enableCache: true,
  /**
   * Disables Stencil's own runtime-injected `<style data-styles>` that hides every `ds-*` tag
   * via `visibility:hidden` until that specific DOM node earns a `.hydrated` class. That
   * per-instance gate leaves elements permanently hidden when a subtree is replaced outside
   * Stencil's own reconciliation (HTMX, `innerHTML`, jQuery `.html()`, ...) — see
   * baloise/design-system#1870. `src/global/_not-defined.scss` replaces it with a `:not(:defined)`
   * gate instead (see docs/adr/0033-not-defined-hydration-visibility.md).
   */
  ...
  outputTargets: [
     ...
     */
    !IS_DS_DEVELOPMENT && !IS_DS_DOCUMENTATION && AngularGenerator(),
    /**
    ...
    before: [
      {
        ...
        async buildStart() {
           ...
           */
          const allTags = await generateComponentTags(__dirname)
