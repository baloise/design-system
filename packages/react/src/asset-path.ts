 ...
 *
 * Deliberately imports `setAssetPath` from `@baloise/ds-core/components`, not from `@baloise/ds-core`:
 * `dist-custom-elements` (`components/`, what this package's generated proxies actually import from) is
 * built with `externalRuntime: false`, which inlines its own separate copy of Stencil's platform runtime
 * rather than importing the shared one `@baloise/ds-core`'s lazy build uses. Calling `setAssetPath`
