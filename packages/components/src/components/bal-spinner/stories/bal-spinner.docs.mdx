import { Story, Canvas, Description } from '@storybook/addon-docs'
import { html } from '../../../stories/utils'
import readme from '../readme.md'
import { Basic } from './bal-spinner.stories'

<bal-doc-banner id="story--components-spinner--basic" status="rebranded,css,visual">Spinner</bal-doc-banner>

<bal-doc-tabs>
  <a href="#story--components-spinner--basic">Overview</a>
  <a href="#examples">Examples</a>
  <a href="#component-api">API</a>
  {/* <a href="#usage">Usage</a> */}
  <a href="#integration">Integration</a>
  <a href="#testing">Testing</a>
</bal-doc-tabs>

<bal-doc-lead>
  <b>Spinner</b> is a animated graphic that lets the user know content is being loaded.
</bal-doc-lead>

<Canvas withSource="open">
  <Story id="components-spinner--basic" />
</Canvas>

<bal-doc-app>
  <bal-button-group class="mb-large" position="center">
    <a class="button is-primary" href="?path=/story/components-spinner--basic">
      Go to playground (Canvas)
    </a>
    <bal-doc-stackblitz template={Basic.parameters.docs.source.code} modules="spinner"></bal-doc-stackblitz>
  </bal-button-group>
</bal-doc-app>

## Examples

<span id="story--components-spinner--loading-button" style={{ opacity: 0 }}></span>

### Loading Button

<Canvas>
  <Story id="components-spinner--loading-button" />
</Canvas>

<span id="story--components-spinner--loading-card" style={{ opacity: 0 }}></span>

### Loading Card

<Canvas>
  <Story id="components-spinner--loading-card" />
</Canvas>

## Component Api

<Description markdown={readme} />

<br />

## Integration

This documentation explains how to implement and use Baloise Design System components across different technologies.

<bal-doc-link-list>
  <a href="?path=/docs/development-getting-started-html5-overview--page">
    <bal-doc-link-list-item template="html5"></bal-doc-link-list-item>
  </a>
  <a href="?path=/docs/development-getting-started-vue-overview--page">
    <bal-doc-link-list-item template="vue"></bal-doc-link-list-item>
  </a>
  <a href="?path=/docs/development-getting-started-angular-overview--page">
    <bal-doc-link-list-item template="angular"></bal-doc-link-list-item>
  </a>
  <a href="?path=/docs/development-getting-started-react-overview--page">
    <bal-doc-link-list-item template="react"></bal-doc-link-list-item>
  </a>
</bal-doc-link-list>

import testing from './testing.md'

<Description markdown={testing} />

<bal-doc-github link="/components/bal-spinner/stories/bal-spinner.docs.mdx"></bal-doc-github>
