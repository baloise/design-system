...
import { BalCalendarCell, BalPointerDate } from './old-datepicker.type'
import {
  isSpaceKey,
  parse,
  format,
...
import { i18nDate } from './utils/bal-datepicker.i18n'
import {
  defaultConfig,
  BalConfigObserver,
  ListenToConfig,
  ...
  implements ComponentInterface, BalConfigObserver, FormInput<string | undefined>, Loggable, BalBreakpointObserver
{
  ...
  render() {
    const block = BEM.block('datepicker-old')
  ...
  renderGrid() {
    const block = BEM.block('datepicker-old-grid')
  ...
  renderHeader() {
    const block = BEM.block('datepicker-old-pagination')
