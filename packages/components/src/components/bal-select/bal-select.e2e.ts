...
describe('bal-select', () => {
  ...
  beforeEach(async () => {
    ...
    // triggerElement = await balSelectElement.find('bal-dropdown-trigger input')

    nativeInputElement = await balSelectElement.find('input')
    selectOptionOne = await balSelectElement.find('bal-select-option[value="v1998"]')
    // selectOptionTwo = await balSelectElement.find('bal-select-option[value="v2000"]')
  })
