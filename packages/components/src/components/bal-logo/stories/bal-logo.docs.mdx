import { Story, Canvas, Description } from '@storybook/addon-docs'
import readme from '../readme.md'

<bal-doc-banner id="story--components-logo--basic">Logo</bal-doc-banner>

<bal-doc-tabs>
  <a href="#story--components-logo--basic">Overview</a>
  <a href="#examples">Examples</a>
  <a href="#component-api">API</a>
  {/* <a href="#usage">Usage</a> */}
  <a href="#integration">Integration</a>
  <a href="#testing">Testing</a>
</bal-doc-tabs>

<bal-doc-lead>
  All the <b>Logos</b> of the Baloise Group are legally protected brands and may not be altered in any way.
</bal-doc-lead>

- [Baloise Logos](https://www.baloise.com/en/home/about-us/who-we-are/images-logos.html)

<Canvas withSource="open">
  <Story id="components-logo--basic" />
</Canvas>

<bal-doc-app>
  <bal-button-group class="mb-6" position="center">
    <a class="button is-info is-outlined" href="?path=/story/components-logo--basic">
      Go to playground (Canvas)
    </a>
  </bal-button-group>
</bal-doc-app>

## Examples

<span id="story--components-logo--baloise-insurance" style={{ opacity: 0 }}></span>

### Baloise Insurance

The insurance logo is available in the languages de, fr, it and en.

<Canvas>
  <Story id="components-logo--baloise-insurance" />
</Canvas>

<span id="story--components-logo--baloise-soba" style={{ opacity: 0 }}></span>

### Baloise Bank SoBa

<Canvas>
  <Story id="components-logo--baloise-soba" />
</Canvas>

## Component Api

<Description markdown={readme} />

<br />

## Integration

This documentation explains how to implement and use Baloise Design System components across different technologies.

<bal-doc-link-list>
  <a href="?path=/docs/getting-started-html5-overview--page">
    <bal-doc-link-list-item template="html5"></bal-doc-link-list-item>
  </a>
  <a href="?path=/docs/getting-started-vue-overview--page">
    <bal-doc-link-list-item template="vue"></bal-doc-link-list-item>
  </a>
  <a href="?path=/docs/getting-started-angular-overview--page">
    <bal-doc-link-list-item template="angular"></bal-doc-link-list-item>
  </a>
  <a href="?path=/docs/getting-started-react-overview--page">
    <bal-doc-link-list-item template="react"></bal-doc-link-list-item>
  </a>
</bal-doc-link-list>

import testing from './testing.md'

<Description markdown={testing} />

<bal-doc-github link="/components/bal-logo/stories/bal-logo.docs.mdx"></bal-doc-github>
