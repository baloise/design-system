import { Component, Host, h, Event, EventEmitter, Prop, Method } from '@stencil/core'
import { BalMode, initStyleMode, updateBalAnimated } from '@/components/utils/config'
import { rIC } from '@/components/utils/helpers'
import { Loggable, Logger, LogInstance } from '@/components/utils/log'
import { balBrowser } from '@/components/utils/browser'
import { balDevice } from '@/components/utils/device'

@Component({
  tag: 'bal-app',
  styleUrls: {
    css: 'bal-app.sass',
  },
})
export class App implements Loggable {
  private focusVisible?: any
  log!: LogInstance

  @Logger('bal-app')
  createLogger(log: LogInstance) {
    this.log = log
  }

  /**
   * @deprecated Mode defines how the styles are loaded. With `css` each component loads his own styles
   * and with `sass` the component styles needs to be imported with the file `components.sass`.
   */
  @Prop({ reflect: true }) mode: BalMode = 'css'

  /**
   * Disables all animation inside the bal-app. Can be used for simplify e2e testing.
   */
  @Prop({ reflect: true }) animated = true

  /**
   * @internal Is `true` when DS components are ready to be shown.
   */
  @Prop({ reflect: true, mutable: true }) ready = false

  /**
   * @internal
   * Tells if the components are ready
   */
  @Event({ bubbles: true, composed: true }) balAppLoad!: EventEmitter<BalEvents.BalAppLoadDetail>

  connectedCallback() {
    initStyleMode(this.mode)
    updateBalAnimated(this.animated)
  }

  componentDidLoad() {
    rIC(async () => {
      this.balAppLoad.emit(true)
      this.ready = true
      import('@/components/utils/focus-visible').then(module => (this.focusVisible = module.startFocusVisible()))
    })
  }

  @Method()
  async setFocus(elements: HTMLElement[]) {
    if (this.focusVisible) {
      this.focusVisible.setFocus(elements)
    }
  }

  render() {
    return (
      <Host
        role="application"
        class={{
          'bal-app': true,
          'bal-app--safari': balBrowser.isSafari,
          'bal-app--touch': balDevice.hasTouchScreen,
          'bal-app--landscape': balDevice.orientation.isLandscape,
          'bal-app--portrait': balDevice.orientation.isPortrait,
        }}
      >
        <slot></slot>
      </Host>
    )
  }
}
