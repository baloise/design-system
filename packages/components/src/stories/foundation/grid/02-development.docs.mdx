...
To work with breakpoints in JavaScript use the `balBreakpoints` utility.

```typescript
import { balBreakpoints } from '@baloise/design-system-components'

// Each breakpoint has a getter function which returns a boolean
balBreakpoints.isMobile
balBreakpoints.isTablet
balBreakpoints.isTouch
balBreakpoints.isDesktop
balBreakpoints.isHighDefinition
balBreakpoints.isWidescreen
balBreakpoints.isFullHD

// Returns a boolean if the given breakpoint is active
balBreakpoints.includes('mobile')
...
```typescript
import { balBrowser } from '@baloise/design-system-components'
...
balBrowser.hasNavigator

// Is true when the document object is defined
balBrowser.hasDocument

// Returns a string with the user agents
balBrowser.userAgent
```

## Device Utilities

```typescript
import { balDevice } from '@baloise/design-system-components'

// Is true if the device supports touchscreen
balDevice.hasTouchScreen

// Is true if it is a native mobile device like a iPhone
balDevice.isMobile

// Is true when the orientation is in portrait mode
balDevice.orientation.isPortrait

// Is true when the orientation is in isLandscape mode
balDevice.orientation.isLandscape
```

#### Angular

In the angular package we provide a `BalOrientationService` with observables. This service listens
on resize events and inform all subscribes.

Just import the `BalOrientationService` from the '@baloise/design-system-components-angular' package
and set is as **public** in the constructor so that we can use it in the template.

```typescript
import { BalOrientationService } from '@baloise/design-system-components-angular'

class MyComponent {
  constructor(public orientation: BalOrientationService)
}
```

In the template access the orientation you need with `orientation.portrait$` or `orientation.landscape$`.

```html
<div *ngIf="orientation.portrait$ | async">... come content ...</div>
```
