...
If **No** than you can easily migrate to the new version, but if **Yes** than go to the installation guide.
...
After the installation update the Design System to 2.x version and replace the import statements, which still use utility functions from the `@baloise/design-system-components` package.
