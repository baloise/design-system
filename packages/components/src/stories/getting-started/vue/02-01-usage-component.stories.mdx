...
After adding the `BaloiseDesignSystem` plugin the components are registered in the framework and can be used directly in your custom vue components.

To get more type-safety you need to register the components from Baloise Design System in your custom vue components.
...
<script lang="ts">
  ...
  onMounted(async () => {
    if (inputRef.value) {
      const inputNativeElement = element<Components.BalInput>(inputRef)
      await wait(100) // wait for the DOM only necessary for setFocus
      await deepReady(inputNativeElement) // wait that the web-component is ready
      await inputNativeElement.setFocus()
    }
  })
</script>
