...
The big advandage of using the template is that the Design System is already included.
...
Follow the instructions of the offical Vue page [Quick Start - With Build Tools](https://staging.vuejs.org/guide/quick-start.html#with-build-tools)
...
Vue project scaffolding tool. Below are the recommanded prompts.
...
If `sass` is yet not istalled run:
