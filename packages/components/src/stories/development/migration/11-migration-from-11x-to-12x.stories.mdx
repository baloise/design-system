...
<Meta
  title="Development/Migration/Migration from 11.x to 12.x"
  parameters={{
    previewTabs: {
      canvas: { hidden: true },
    },
...
- [Stencil Update v2.19.2](https://github.com/ionic-team/stencil/releases/tag/v2.19.2)
...
To reduce the initial page load we now lazy load the component stylesheets. This reduces our
global style file. The styles of the components are loaded when they are used.

This change can also be added as non-breaking with adding the prop `mode` to the `bal-app` component.

```html
<bal-app mode="sass"> ...content </bal-app>
```

That tells the Design System to use the SASS styles and not the inline css styles.
...
+@import 'node_modules/@baloise/design-system-components/src/styles/global.components';\n
...
All the below listed alias are removed, so please use the direct value instead.
...
We only support the spacing concept with t-shirt size like `m-large` or `pt-x-small` instead of the numbers.
However, the can be activated with the SASS variable `$spacing-legacy-numbering`, but this will increase the css payload heavily.
...
In version 12 we decoupled bal-number-input from bal-input. The props `decimal` and `numberInput` are removed from the component `bal-input`.
This leads to a small payload for the `bal-input` component.
...
The new component carousel replaces the `bal-slider`, `bal-product-slider` and `bal-image-slider`.
...
+   <bal-carousel-item src="https://i.picsum.photos/id/703/1280/720.jpg?hmac=sICuW9WVQ1Ul6j4mTHDPbj43bHqe062gU35Blq2V-MI"></bal-carousel-item>
    ...
-</bal-image-slider>
+</bal-carousel>`}
/>

<bal-doc-github link="/stories/development/migration/10-migration-from-11x-to-12x.stories.mdx"></bal-doc-github>
