import { Meta } from '@storybook/addon-docs'

<Meta
  title="Development/Migration/Migration from 8.x to 9.x"
  parameters={{
    previewTabs: {
      canvas: { hidden: true },
    },
  }}
/>

# Migration from 8.x to 9.x

The following workflow walks you through the migration steps.
Note that the actual steps required for your project may vary, and these steps should be treated as general guidance rather than strict instructions.

## Tabs, Accordion and Popover

The components `tabs`, `accordion` and `popover` now can be used with two-way bindings (ngModel or v-model).

```html
<!-- angular -->
value = 'tab-a'
<bal-tabs [(ngModel)]="value">...</bal-tabs>

<!-- vue -->
value = ref('tab-a')
<bal-tabs v-model="value">...</bal-tabs>
...
```

## Dropdown / Popover

`bal-dropdown` has been renamed as bal-popover.

## Hint

Method `open` has been renamed as `present`.
Method `close` has been renamed as `dismiss`.

## Form fields

All fields are set as expanded. All expanded attributes in the form section can be removed.

## Modal

The `bal-modal-footer` and `bal-modal-actions` have been removed due the adjustment of the scroll behavior.

```html
<!-- - old -->
<bal-modal>
  <bal-modal-header>Modal Title</bal-modal-header>
  <bal-modal-body>
    <p>
      Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore
      magna aliqua.
    </p>
  </bal-modal-body>
  <bal-modal-footer>
    <bal-modal-actions>
      <bal-button color="link">Cancel</bal-button>
      <bal-button color="primary">Okay</bal-button>
    </bal-modal-actions>
  </bal-modal-footer>
</bal-modal>

<!-- + new -->
<bal-modal>
  <bal-modal-header>Modal Title</bal-modal-header>
  <bal-modal-body>
    <p>
      Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore
      magna aliqua.
    </p>
    <bal-button-group position="right">
      <bal-button color="link">Cancel</bal-button>
      <bal-button color="primary">Okay</bal-button>
    </bal-button-group>
  </bal-modal-body>
</bal-modal>
...
```

<bal-doc-github link="/stories/development/migration/08-migration-from-8x-to-9x.stories.mdx"></bal-doc-github>
