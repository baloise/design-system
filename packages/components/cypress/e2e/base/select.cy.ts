...
describe('Select', () => {
  ...
  describe('multiple', () => {
    it('should ', () => {
      ...
      cy.get(page.multiple).balSelectFindOptions().first().click({ force: true })
      cy.get(page.multiple).balSelectFindOptions().eq(1).click({ force: true })
      cy.get(page.multiple).balSelectFindOptions().eq(2).click({ force: true })
      cy.get(page.multiple).should('have.value', ['Iron Man'])
      cy.get(page.multiple).balSelectFindChips().first().contains('Iron Man').click({ force: true })
