...
describe('bal-navigation', () => {
  function testNavigationOnDesktop(platform: Platforms) {
    describe(platform, () => {
      ...
      beforeEach(() => {
        cy.platform(platform).getComponent('bal-doc-app').wait(100)
      })

      it('closed menu on top', () => {
        cy.compareSnapshot(`navigation-desktop-${platform}-closed-top`, compareSnapshotOptions(platform))
      })
      ...
      it('open menu', () => {
        ...
        cy.compareSnapshot(`navigation-desktop-${platform}-open`, compareSnapshotOptions(platform))
      })
      it('open popover', () => {
        cy.get('.bal-nav__meta__end').find('button').first().click()
        cy.compareSnapshot(`navigation-desktop-${platform}-popover-open`, compareSnapshotOptions(platform))
      })
    })
  }
  ...
  function testNavigationOnTouch(platform: Platforms) {
    describe(platform, () => {
      ...
      it('closed menu on top', () => {
        cy.compareSnapshot(`navigation-touch-${platform}-closed-top`, compareSnapshotOptions(platform))
      })
      it('closed menu on bottom', () => {
        cy.scrollTo('bottom')
      ...
      it('open menu', () => {
        ...
        cy.compareSnapshot(`navigation-touch-${platform}-open`, compareSnapshotOptions(platform))
      })
      it('open popover', () => {
        cy.get('.bal-nav__foot-mobile').find('button').first().click().wait(100)
        cy.compareSnapshot(`navigation-touch-${platform}-popover-open`, compareSnapshotOptions(platform))
