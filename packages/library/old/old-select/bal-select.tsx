...
export class OldSelect {
   ...
   */
  @Prop() multiple = false ok

  /**
   * The name of the control, which is submitted with the form data.
   */
  @Prop() name: string = this.inputId ok

  /**
   * If `true` the filtering of the options is done outside of the component.
   */
  @Prop() noFilter = false ok

  /**
   * The tabindex of the control.
   */
  @Prop() balTabindex: number = 0 ok

  /**
   * If `true` the component uses the whole width.
   */
  @Prop() expanded = false ok

  /**
   * Set this to `true` when the component is placed on a dark background.
   */
  @Prop() inverted = false todo
