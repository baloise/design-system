...
| `balTeaserStepChange` | Emitted when the changes has finished.    | `CustomEvent<BalTeaserStepOption>` |
| `balTeaserStepClick`  | Emitted when the step circle is clicked.  | `CustomEvent<BalTeaserStepOption>` |
...
graph TD;
  ...
  style bal-teaser-steps fill:#f9f,stroke:#333,stroke-width:4px
```
