...
interface MultiSelectButtonAccessorType
  extends Andable<MultiSelectButtonAccessorType>,
    Blurable<MultiSelectButtonAccessorType>,
    Selectable<MultiSelectButtonAccessorType>,
    Containable<MultiSelectButtonAccessorType>,
    Existable<MultiSelectButtonAccessorType>,
    Clickable<MultiSelectButtonAccessorType>,
    Shouldable<MultiSelectButtonAccessorType>,
    Visible<MultiSelectButtonAccessorType>,
    Disableable<MultiSelectButtonAccessorType>,
    NthSelectable<MultiSelectButtonAccessorType>,
    Attributable<MultiSelectButtonAccessorType>,
    Urlable<MultiSelectButtonAccessorType>,
    Findable<MultiSelectButtonAccessorType>,
    Waitable<MultiSelectButtonAccessorType>,
    Invokable<MultiSelectButtonAccessorType>,
    Thenable<MultiSelectButtonAccessorType>,
    Lengthable<MultiSelectButtonAccessorType>,
    Eachable<MultiSelectButtonAccessorType> {}
...
export const MultiSelectButtonSelectableMixin: Mixin = <T>({ selector, creator }: MixinContext<T>) => ({
  select: (indexes: number | number[]) => {
    ...
    cy.get(selector).within(() => {
      ;(indexes as number[]).forEach((index: number) => cy.get(`bal-checkbox`).eq(index).check())
    })
    return creator()
  },
  ...
  assertIsSelected: (indexes: number | number[]) => {
    ...
    cy.get(selector).within(() => {
      ;(indexes as number[]).forEach((index: number) => cy.get(`bal-checkbox`).eq(index).should('be.checked'))
    })
    return creator()
  },
})
...
export const MultiSelectButtonAccessor: Accessor<MultiSelectButtonAccessorType> =
  createAccessor<MultiSelectButtonAccessorType>(
    AndableMixin,
    BlurableMixin,
    MultiSelectButtonSelectableMixin,
    ContainableMixin,
    ClickableMixin,
    ExistableMixin,
    ShouldableMixin,
