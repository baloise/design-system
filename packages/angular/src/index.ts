...
export function provideDesignSystem(config: DsAngularConfig = {}): EnvironmentProviders {
  return makeEnvironmentProviders([
    provideEnvironmentInitializer(() => {
      ...
      //
      // Deliberately imports `setAssetPath` from `@helvetia-design/core/components` here, not from
      // `@helvetia-design/core`/`@stencil/core` inside `initializeDesignSystem` itself: `dist-custom-elements`
      // (`components/`, what `@helvetia-design/angular`'s generated proxies actually import from) is built with
      // `externalRuntime: false`, which inlines its own separate copy of Stencil's platform runtime rather
      // than importing the shared one `@helvetia-design/core`'s lazy build uses. Calling `setAssetPath` through
