...
describe('bal-input-date.cy.ts', () => {
  ...
  it('should render the component with the initial state', () => {
    ...
    cy.get('bal-input-date').find('input').should('exist')
    cy.get('bal-input-date').find('input').should('not.have.class', 'is-disabled')
    cy.get('bal-input-date').find('input').should('not.have.class', 'is-danger')
    cy.get('bal-input-date').find('input').should('not.have.class', 'is-clickable')
    cy.get('bal-input-date').find('input').should('not.have.class', 'has-icon-right')
    cy.get('bal-input-date').find('input').should('not.be.disabled')
    cy.get('bal-input-date').find('input').should('not.have.attr', 'aria-disabled')
    cy.get('bal-input-date').find('input').should('not.have.attr', 'required')
    cy.get('bal-input-date').find('input').should('not.have.attr', 'placeholder')
    cy.get('bal-input-date').find('input').should('have.value', '')
  })

  it('should update the value when typing', () => {
    cy.get('bal-input-date').find('input').type('20021988', { delay: 10 })
