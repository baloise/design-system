# 7. ds-slider replaces the native `<input type="range">` with noUiSlider
...
`ds-slider` originally wrapped a native `<input type="range">` inside
...
[ADR-0010](./0010-ds-slider-change-commit.md) (commit on native
`change`, not blur).
...
3. Wrap the noUiSlider instance in `SliderPickerController`
   (`slider.picker.ts`), mirroring the existing `SelectPickerController`
...
5. Map noUiSlider's own events onto the existing public event contract:
   ...
   `SliderPickerController.setValue()` additionally ensures an
...
- `FormControl`'s onBlur/onInput/nativeEl model now has two known
  exceptions (`ds-select`, `ds-slider`) rather than one; a future
