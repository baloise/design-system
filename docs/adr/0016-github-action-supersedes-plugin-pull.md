# 16. A GitHub Action, not the plugin, owns Pull (from Code)
...
[figma-token-sync-plugin-plan.md](../plans/figma-token-sync-plugin-plan.md) originally scoped Pull (from Code) as
...
[§8 Future Extensions](../plans/figma-token-sync-plugin-plan.md#8-future-extensions)
...
([ADR-0011](0011-figma-variable-identity-key.md)), brand-mode mapping
([ADR-0012](0012-brand-modes-not-collections.md)), native aliasing
([ADR-0013](0013-native-variable-aliasing.md)), and
`.figma-sync-state.json` baseline ([ADR-0015](0015-git-committed-sync-baseline.md)) —
...
- Pull can no longer be deferred or reviewed by a human before it happens —
  ...
  in [ADR-0017](0017-direct-commit-variableid-backfill.md) and
  [ADR-0018](0018-non-blocking-conflict-check.md) exist because of this.
