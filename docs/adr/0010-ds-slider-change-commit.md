# 10. ds-slider commits its value on the native `change` event, not on blur
...
`ds-slider` wraps a native `<input type="range">`. Range inputs already
...
`ds-slider` does not reuse `FormControl.onBlur()` for its commit logic.
Instead:
...
- Someone reading `ds-slider` after having learned the `FormControl`
