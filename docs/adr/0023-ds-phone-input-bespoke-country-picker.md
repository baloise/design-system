# 23. ds-phone-input uses a bespoke country picker instead of extending ds-select
...
`ds-phone-input` needs a country selector showing a flag, localized country
...
`ds-phone-input` instance (unclear DOM/perf cost, and those elements would
...
- Wrap `ds-phone-input`'s number field inside `ds-input` and bolt the picker
...
`ds-phone-input`: a `<button>` trigger showing the selected flag + calling
...
- `ds-phone-input` owns and maintains its own picker implementation
