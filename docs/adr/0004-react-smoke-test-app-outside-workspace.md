...
Superseded by [ADR-0009](0009-integration-apps-as-workspace-members.md)
...
added as a workspace member, `@helvetia-design/react` and `@helvetia-design/core`
...
an external consumer actually experiences: installing `@helvetia-design/react`
...
`pnpm-workspace.yaml`). `test/react/build.sh` builds `@helvetia-design/core`
and `@helvetia-design/react` from their `dist/` output and links them into the
