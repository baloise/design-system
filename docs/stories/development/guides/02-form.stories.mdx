...
<Meta title="Development/From" />

<Banner label="From" section="Development" color="red" />
...
1. The Design System works with Angular to visually indicate to the user when they have entered incorrect information by setting
the invalid form field to red. This is achieved by listening to the form control state, specifically when it is both invalid and touched.
...
2. Angular package offers `balAutoFocus` directive which brings focus to the element.
...
3. In the component class wen can define the validators for the form control.
