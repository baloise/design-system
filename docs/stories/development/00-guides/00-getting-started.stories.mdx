...
<Banner label="Getting Started" section="Development" color="red" />
...
For **application components**, we recommend using
`@baloise/design-system-css/sass/mixins` to gain access to SASS mixins, such as breakpoints.
...
We recommend to import the `BaloiseDesignSystemModule` also in you `SharedModule` to provide the components and the value accessors.
...
<Footer>
  <LinkCards>
    <LinkCard pageTitle={'Font Installation'} description={'TODO'} />
    <LinkCard pageTitle={'Favicons Installation'} description={'TODO'} />
    <LinkCard
      pageTitle={'Development/Component'}
      description={
        'By using the standardized web platform APIs and Web Components we achieved that the components of the Baloise Design System can be used across any framework or no framework at all'
      }
    />
    <LinkCard
