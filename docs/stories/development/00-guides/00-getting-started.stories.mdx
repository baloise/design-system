...
<Code language="bash" noPreview code={`npm install @baloise/design-system-components-schematics`} />

<InfoQuote>
If the projects requires translatiojns, then install [\{TRANSLOCO\}](https://ngneat.github.io/transloco/) before the Baloise Design System.
...
```bash
ng add @baloise/design-system-schematics:ng-add
# On an nx workspace
nx g @baloise/design-system-schematics:ng-add
```
