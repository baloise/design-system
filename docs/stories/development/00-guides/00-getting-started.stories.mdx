...
The `@baloise/ds-vue` dependency includes the plugin `BaloiseDesignSystem`, which loads by default the polyfills and defines the components.
Moreover, it adds the controllers to your vue instance.

To install the Baloise Design System run the following command.

```
npm add -E @baloise/ds-vue
```
...
```typescript
import './main.scss'
import { createBaloiseDesignSystem } from '@baloise/ds-vue'
...
<script setup lang="ts">
  import { BalApp, BalFooter, BalHeading, BalButton } from '@baloise/ds-vue'
</script>
