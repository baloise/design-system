import { Canvas, Markdown, Meta } from '@storybook/addon-docs/blocks'
import { Banner, Footer, Lead, LinkCard, LinkCards, PlaygroundBar, StoryHeading } from '../../../.storybook/blocks'
import * as TextareaStories from './bal-textarea.stories'

<Meta of={TextareaStories} />

<StoryHeading of={TextareaStories.Basic} hidden></StoryHeading>

<Banner of={TextareaStories} />

<Lead>**Textarea** allows a user to write and edit large texts.</Lead>

<Canvas of={TextareaStories.Basic} sourceState="shown" />

<PlaygroundBar of={TextareaStories.Basic}></PlaygroundBar>

{/* STORIES */}
{/* ------------------------------------------------------ */}

<StoryHeading of={TextareaStories.FieldControl}></StoryHeading>

Combine the component with the `bal-field` components to add a label, validation message and more.

<Canvas of={TextareaStories.FieldControl} />

{/* ------------------------------------------------------ */}

<StoryHeading of={TextareaStories.InvalidTextarea}></StoryHeading>

<Canvas of={TextareaStories.InvalidTextarea} />

{/* ------------------------------------------------------ */}

<StoryHeading of={TextareaStories.NativeTextarea}></StoryHeading>

<Canvas of={TextareaStories.NativeTextarea} />

{/* ------------------------------------------------------ */}

## Component API

import api from './api.md?raw'

<Markdown>{api}</Markdown>

## Integration

import integration from '../../snippets/integration.md?raw'

<Markdown>{integration}</Markdown>

import theming from './theming.md?raw'

<Markdown>{theming}</Markdown>



<Footer>
  <LinkCards>
    <LinkCard
      pageTitle="Development/Form"
      description="The indispensable form controls, designed for maximum clarity. Form elements are used in combination with the CSS grid system"
    />
  </LinkCards>
</Footer>
