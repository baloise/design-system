...
11. **Figma pull is Base-layer only**, no brand/ERV override support — same scope limit as shadow's pull, and consistent with decision 8 (no brand-level composite tokens exist to pull anyway in this pass).
...
- Any composite border token at brand/ERV-override level.
