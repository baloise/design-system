...
Status: proposed · Owner: packages/tokens · Related: [ADR 0011](../adr/0011-figma-variable-identity-key.md), [ADR 0012](../adr/0012-brand-modes-not-collections.md), [ADR 0013](../adr/0013-native-variable-aliasing.md), [ADR 0014](../adr/0014-git-data-api-atomic-commits.md), [ADR 0015](../adr/0015-git-committed-sync-baseline.md), [ADR 0016](../adr/0016-github-action-supersedes-plugin-pull.md), [ADR 0017](../adr/0017-direct-commit-variableid-backfill.md), [ADR 0018](../adr/0018-non-blocking-conflict-check.md), [ADR 0019](../adr/0019-pull-auto-deletes-figma-variables.md), [ADR 0020](../adr/0020-figma-sync-action-standalone-script.md)

## 1. Purpose

Automate Pull (from Code) — see [packages/tokens/CONTEXT.md](../../packages/tokens/CONTEXT.md) —
...
- **`figma-conflict-check.yml`** — runs while the PR is open. Non-blocking
  visibility into Conflicts (ADR-0018).
...
| Brand scope               | All brands from day one — Base mode plus every `<Brand>.tokens.json`'s mode, in the same run (ADR-0012).                                                                                                                                                                                                                                                                                                                           |
| Metadata location         | Inline `$extensions.com.figma.{variableId,scopes}` on the token (existing convention, [ADR-0011](../adr/0011-figma-variable-identity-key.md)) — not `.figma-sync-state.json`, which stays sync bookkeeping only ([ADR-0015](../adr/0015-git-committed-sync-baseline.md)).                                                                                                                                                                |
| New-variableId write-back | Direct commit to `next`, bundled with the `.figma-sync-state.json` update, one atomic Git Data API commit ([ADR-0014](../adr/0014-git-data-api-atomic-commits.md), [ADR-0017](../adr/0017-direct-commit-variableid-backfill.md)).                                                                                                                                                                                                        |
| Token deletion            | Auto-delete the matching Figma Variable ([ADR-0019](../adr/0019-pull-auto-deletes-figma-variables.md)).                                                                                                                                                                                                                                                                                                                               |
| Conflict handling         | Comment-only, non-blocking ([ADR-0018](../adr/0018-non-blocking-conflict-check.md)).                                                                                                                                                                                                                                                                                                                                                  |
| Reference handling        | Native Figma variable aliases, not flattened literals ([ADR-0013](../adr/0013-native-variable-aliasing.md)) — two-pass write (create all variables, then bind aliases), same ordering constraint the plugin plan identified.                                                                                                                                                                                                          |
| Code location             | Standalone script, not shared with `apps/toky` ([ADR-0020](../adr/0020-figma-sync-action-standalone-script.md)).                                                                                                                                                                                                                                                                                                                      |
...
| GitHub write              | Same bot credential already used elsewhere in `.github/workflows` (`PRE_RELEASE_GITHUB_TOKEN` or equivalent) — needs branch-protection bypass on `next`, scoped to the id-backfill commit path (ADR-0017).                                                                                                                                                                                                                         |
  ...
  sync:
    ...
    steps:
      ...
      - name: Pull tokens to Figma
        ...
        # 4. Delete Figma Variables for tokens no longer present (ADR-0019).
...
- Wire the auto-delete path (ADR-0019) with an explicit test: a token
...
- **Figma REST API access tier**: the Variables API is a paid-plan/Enterprise
  ...
  after.
- **Bot branch-protection bypass** (ADR-0017): needs to be granted on
