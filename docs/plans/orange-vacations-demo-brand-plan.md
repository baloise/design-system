# Plan: First demo brand — Orange Vacations
...
overrides `White`) — this placeholder brand was later removed once Orange
Vacations shipped. Orange Vacations is the first **real, complete** brand
override — the goal is to prove the existing pipeline end-to-end
(colors → buttons → typography), not to build new infrastructure.
...
human** from the partner's brand guidelines — not scraped or guessed by an
agent.
...
- Toky's brand-creation flow writes `OrangeVacations.tokens.json` **and**
  auto-patches `packages/tokens/src/index.ts`'s `brands` array in the same PR
...
Override only:
1. **Colors** — global palette tokens Orange Vacations' brand relies on
   (primary/accent colors used by `Button`'s alias/component tokens).
...
- [ ] `OrangeVacations.tokens.json` created via Toky, with real color,
      button, and typography-family overrides sourced from the partner's
      brand guidelines.
- [ ] `packages/tokens/src/index.ts` brands array includes
      `'OrangeVacations'` (handled automatically by Toky's submit).
...
- [ ] New visual test page:
      `packages/core/src/foundation/theming/orange-vacations.visual.html`
      (new `theming/` subfolder — first of its kind), wrapped in
      `data-theme="orange-vacations"`, showing a collection of components (at
      minimum: all `ds-button` variants/states, heading, and body text) so
      the brand override is visible in one place.
- [ ] Matching Playwright visual regression test:
      `packages/core/src/foundation/theming/orange-vacations.visual.play.ts`
      (+ generated snapshots), following the same pattern as
      `colors.visual.play.ts` / `typography.visual.play.ts`.
- [ ] Manual spot-check: Storybook `Theme` global set to `orange-vacations`
      against a handful of existing component stories, confirming colors/
      buttons/typography swap correctly with no visual regressions elsewhere.
...
- No scraping/automation to extract Orange Vacations' brand values —
  human-sourced.
