...
@ProxyCmp({
  inputs: ['card', 'closeIcon', 'closeLabel', 'isActive', 'openIcon', 'openLabel', 'type'],
...
@ProxyCmp({
  inputs: ['border', 'flat', 'inverted', 'padded', 'square', 'teaser', 'type'],
})
@Component({
  selector: 'bal-card',
  changeDetection: ChangeDetectionStrategy.OnPush,
...
export class BalInput {
  ...
  constructor(c: ChangeDetectorRef, r: ElementRef, protected z: NgZone) {
    ...
    this.el.addEventListener('balInput', (event: any) => {
      let emittedValue = event
      if ('detail' in event) {
        emittedValue = event.detail
      }
      console.log('balInput', emittedValue)
      this.balInput.emit(emittedValue)
    })
  }
}
