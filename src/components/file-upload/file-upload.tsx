...
export class FileUpload {
   ...
   */
  @Event({
    eventName: "balFileUploadRejectedFile",
    composed: true,
    cancelable: true,
    bubbles: true,
  })
  ...
  handleFiles(files: FileList): void {
    if (!this.disabled) {
      ...
      for (let index = 0; index < files.length; index++) {
        ...
        const duplicatedFiles = list.filter(
          (f) =>
            f.size === file.size &&
            f.name === file.name &&
            f.type === file.type,
  ...
  render() {
    return (
      <Host
        class={["bal-file-upload", this.disabled ? "is-disabled" : ""].join(
          " ",
        )}
      >
        <div class="file">
        ...
        <bal-list disabled border>
          {this.files.map((file, index) => (
            <bal-list-item>
              ...
              >
                <bal-icon size="medium" name="trash"></bal-icon>
              </bal-list-item-icon>
            </bal-list-item>
          ))}
        </bal-list>
