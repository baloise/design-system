...
Each component lives in `packages/core/src/components/<bal-component>/`:

```
bal-button/
  bal-button.tsx            # Stencil component — props, events, render()
  bal-button.interfaces.ts  # TypeScript types/enums specific to this component
  bal-button.host.scss      # Shadow DOM styles — scoped inside :host {}; imports
                            #   bal-button.style.scss with ($use-host: true)
  bal-button.style.scss     # Shared CSS rules — used by BOTH the web component
                            #   (via host.scss) and the global stylesheet (via
                            #   packages/styles). This is what enables hybrid mode.
  test/
    bal-button.spec.ts              # Vitest unit tests
    bal-button.component.play.ts    # Playwright component and PO tests
    bal-button.style.html           # Static HTML fixture for visual/style tests
    bal-button.visual.html          # Visual regression baseline HTML
    bal-button.visual.play.ts       # Playwright visual test
    bal-button.a11y.play.ts         # Playwright accessibility test
```

**Hybrid component model:** `*.style.scss` is the single source of CSS truth. It is:
...
--_button-color-text          (private — used in CSS rules)
  └─ var(--button-color-text        (public — consumer override)
      , var(--mod-button-color-text  (modifier — set by .is-primary, .is-sm, etc.)
          , var(--bal-button-color-primary-base-text)))  (design token default from Figma)
```
...
| Design token | `--bal-component-prop` | `packages/tokens` (imported from Figma) | The default value; changed only via token updates               |
...
Each component has a dedicated Storybook entry under `docs/src/components/<bal-component>/`:

```
bal-tag/
  bal-tag.stories.ts   # Story definitions — uses StoryFactory + withRender helpers
  bal-tag.mdx          # MDX page — imports stories, assembles the doc page
...
All components must meet **WCAG 2.1 AA** compliance. This includes:
...
Every component needs a **Page Object (PO)** in `packages/playwright/src/lib/components/<bal-component>.po.ts`. The PO extends `PageObject` and exposes typed locators and helper methods (`clickClose()`, `assertToContainText()`, etc.). It is the only way tests interact with a component.

```ts
// packages/playwright/src/lib/components/bal-tag.po.ts
...
Every component also needs a `*.component.play.ts` test. This serves two purposes: it verifies the PO itself works, and for components with JS logic it also asserts that events fire the correct number of times and carry the correct `detail` payload.

```ts
// test/bal-tag.component.play.ts
...
Summary of required per-component test files:
