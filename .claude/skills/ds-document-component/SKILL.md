...
>
> **Why a static string, not a computed title?** Storybook's CSF/MDX indexer requires `title` to
> be a static string literal — a helper function call (e.g. deriving the category from
> `import.meta.url`) fails to index with `CSF: unexpected dynamic title`. Write the category
