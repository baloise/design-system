...
async function gatherUserInput() {
  ...
  const purpose = await prompt(
    'Component purpose (e.g., "Renders a clickable element"):'
  );
  ...
  // Q4: Props
  const propsInput = await prompt(
    'Props (comma-separated, e.g., "label: string = \\'\\', disabled: boolean = false"):'
  );
  responses.props = parsePropInput(propsInput);

  // Q5: Events
  const eventsInput = await prompt(
    'Events (comma-separated, e.g., "dsClick, dsChange"):'
  );
  ...
  if (hasSubcomponents) {
    const subInput = await prompt(
      'Subcomponent names (comma-separated, e.g., "tab, tab-content"):'
    );
  ...
  // Q7: Variants
  const variantsInput = await prompt(
    'Variants (comma-separated, e.g., "primary, secondary, danger"):'
  );
...
function writeComponentFiles(componentName, files, subcomponents) {
  ...
  fs.writeFileSync(
    path.join(componentPath, `${componentName}.interfaces.ts`),
    files.interfaces
  );
  fs.writeFileSync(
    path.join(componentPath, `${componentName}.host.scss`),
    files.scss
  );
  fs.writeFileSync(
    path.join(testPath, `${componentName}.visual.html`),
    files.html
  );
  ...
  subcomponents.forEach(subcomponent => {
    ...
    fs.writeFileSync(
      path.join(subPath, `${subcomponent}.tsx`),
      files[`${subcomponent}/tsx`]
    );
    fs.writeFileSync(
      path.join(subPath, `${subcomponent}.interfaces.ts`),
      files[`${subcomponent}/interfaces`]
    );
    fs.writeFileSync(
      path.join(subPath, `${subcomponent}.host.scss`),
      files[`${subcomponent}/scss`]
    );
  });
