...
test('ds-phone-input fires dsInput and updates Angular state', async ({ page }) => {
  await page.goto('/')
...
test('ds-phone-input reactive form: initial value, value update, disabled, and touched+invalid', async ({ page }) => {
  await page.goto('/')
...
test('ds-phone-input reactive form: country change updates the form control immediately, without blur', async ({
  page,
}) => {
  await page.goto('/')
  ...
  // Asserted with no blur in between: proves `ds-phone-input` itself fires `dsChange` from a country
...
test('ds-phone-input reactive form: autoInvalidOff suppresses automatic invalid state', async ({ page }) => {
  await page.goto('/')
