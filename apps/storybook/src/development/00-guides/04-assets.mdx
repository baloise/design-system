...
"scripts": {
  ...
  "copy:fonts": "copyfiles --flat node_modules/@baloise/ds-assets/src/fonts/* src/assets/fonts",
}
```
...
```ts
import { dsMapTypeStyles } from '@baloise/ds-maps'

const myMap = new google.maps.map(myMapElement, {
  ...
  styles: dsMapTypeStyles
})
```
...
```ts
import { dsMapMarkerDefault } from '@baloise/ds-maps'

new google.maps.Marker({
  position: { lat: 36.6163, lng: -100.61 },
  map,
  icon: {
    url: dsMapMarkerDefault,
  },
  title: 'My Marker',
})
```
