...
<Lead>
  The Helvetia Design System offers a comprehensive list of utility functions designed specifically for testing web
  components.
</Lead>
...
Open the "component-index.html" file and integrate the `ds-app` component.
...
```html
<div class="p-large">
  <h1 class="ds-title">Native Title</h1>
  <ds-heading>Component Title</ds-heading>
</div>
```
...
describe('hello.component.cy.ts', () => {
  it('playground', () => {
    ...
    cy.get('ds-heading').contains('Component Title')
  })
})
```
...
describe('Accordion', () => {
  ...
  it('should ...', () => {
    ...
    cy.get(accordion).dsAccordionIsClosed()
    cy.get(accordion).click().dsAccordionIsOpen()
    ...
    cy.get(accordion).click().dsAccordionIsClosed()
  })
})
```
...
```html
<ds-button data-testid="my-button"></ds-button>
```
