...
Import the `useDesignSystem` function and pass the default values of the config.

<Code language="ts" code={`
import { useDesignSystem } from '@baloise/ds-react'
...
`initialize`/`useDesignSystem` isn't practical, a subset of the config can
...
<InfoQuote>
  ...
  `userConfig` object passed to `initialize`/`useDesignSystem`, which always
  wins.
</InfoQuote>
